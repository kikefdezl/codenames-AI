...
pub fn play_spymaster_game() {
    ...
    loop {
        ...
        let result: Vec<f32> = compute_word_to_words_similarity(&reference_word, &remaining_words)
            .expect("Couldn't get result from AI model");
        let max_words: Vec<String> = get_n_max_words(&remaining_words, &result, n_words_referenced);
